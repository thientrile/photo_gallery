...
#### 🗑️ **Xóa Album**
```bash
# Xóa album và unlink tất cả ảnh
DELETE /api/gallery/album/:albumId

# Example
DELETE /api/gallery/album/123456
```

**Hành vi khi xóa album:**
- ✅ Kiểm tra quyền sở hữu (chỉ owner có thể xóa)
- ✅ **Unlink tất cả ảnh** trong album (set `img_albumId = null`)
- ✅ **Ảnh không bị xóa**, chỉ loại bỏ khỏi album
- ✅ Xóa album khỏi database
- ✅ Trả về thông tin chi tiết: tên album, số ảnh đã unlink

**Response format:**
```json
{
  "status": "success",
  "message": "Album 'Kỳ Nghỉ Hè 2025' đã được xóa thành công",
  "metadata": {
    "albumId": 123456,
    "albumTitle": "Kỳ Nghỉ Hè 2025", 
    "imagesUnlinked": 15
  }
}
```
...
**Error Cases:**
- `400 Bad Request`: Album không tồn tại hoặc không có quyền
- `400 Bad Request`: Album ID không hợp lệ (phải là số dương)
...
# 7. Delete album (and unlink all images)
curl -X DELETE http://localhost:3002/api/gallery/album/789 \
  -H "Authorization: Bearer YOUR_TOKEN"

# 8. Test Pagination - Images
curl -X GET "http://localhost:3002/api/gallery/image/?page=1&limit=5" \
  -H "Authorization: Bearer YOUR_TOKEN"

# 9. Test Pagination - Albums  
...
# 11. Test Large Limit (will be capped at 100)
